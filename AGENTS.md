...
- CSS: Tailwind v4 `@tailwindcss/cli` with `@theme` + `@layer components` only (no utilities/preflight in the shipped file). Consumers import compiled CSS; they must not Tailwind-scan this package.
